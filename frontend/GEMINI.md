# Real-time Board (Frontend)

## Project Overview
This is the frontend application for a "Real-time Board" project. It is a modern Single Page Application (SPA) built with React and Vite, designed for real-time collaboration and canvas-based interaction. It utilizes `shadcn/ui` for high-quality, customizable UI components.

**Key Features (Inferred from Dependencies):**
*   **Real-time Collaboration:** Powered by `yjs` and `y-websocket`.
*   **Canvas/Whiteboard:** Utilizes `konva` and `react-konva` for high-performance 2D graphics.
*   **State Management:** Uses `zustand` for a simplified global state.
*   **router:** Uses `tanstack router` 
*   **Data Fetching:** `axios` and `@tanstack/react-query` for API interactions.
*   **Styling:** `tailwindcss` for utility-first CSS, augmented by `shadcn/ui`.
*   **Icons:** `lucide-react`.
...
*   **Imports:** Use the `@/` alias for absolute imports from the `src` directory (e.g., `import Button from '@/components/Button'`).
*   **State:** Prefer `zustand` for global client state and `react-query` for server state.
*   **Canvas:** Use `react-konva` components for drawing on the canvas.
...
> Tailwind CSS + shadcn/ui + Lucide React — Clean & Minimal Dark Theme
...
| Token           | Value           | Usage                    |
|-----------------|-----------------|--------------------------|
| `bg-base`       | `neutral-900`   | Page background          |
| `bg-surface`    | `neutral-800`   | Card, modal, input       |
| `bg-elevated`   | `neutral-700`   | Hover states, divider    |
| `text-primary`  | `neutral-200`   | Heading, body text       |
| `text-secondary`| `neutral-400`   | Caption, placeholder     |
| `text-muted`    | `neutral-500`   | Disabled, hint           |
| `accent`        | `yellow-500`    | CTA, links, focus ring   |
| `accent-hover`  | `yellow-400`    | Hover state              |
| `accent-active` | `yellow-600`    | Active/pressed state     |
| `border`        | `neutral-700`   | All borders (1px)        |
...
```
Luôn dùng: border border-neutral-700
Độ dày: 1px (mặc định)
```
...
| Color        | Inherit hoặc `text-neutral-400`  |
...
| Card    | `hover:border-neutral-600`|
| Link    | `hover:text-yellow-500`   |
| Icon    | `hover:text-neutral-200`  |

### Active / Pressed

| Element | Class                  |
|---------|------------------------|
| Button  | `active:bg-yellow-600` |
...
| State    | Border Class         | Extra                     |
|----------|----------------------|---------------------------|
| Default  | `border-neutral-700` | `bg-neutral-800`          |
| Focus    | `border-yellow-500`  | Focus ring                |
| Error    | `border-red-500`     | `text-red-400` message    |
| Success  | `border-green-500`   | `text-green-400` message  |
| Disabled | `border-neutral-700` | `opacity-50 bg-neutral-900`|

### Form Text

| Element     | Classes                         |
|-------------|---------------------------------|
| Label       | `text-sm text-neutral-400 mb-1.5`|
| Placeholder | `placeholder:text-neutral-500`  |
| Helper      | `text-xs text-neutral-500 mt-1` |
| Error msg   | `text-xs text-red-400 mt-1`     |
...
| Slow   | `duration-300` | Modal, drawer     |
...
Skeleton: animate-pulse bg-neutral-700
```
...
Avatar: rounded-full
Placeholder: bg-neutral-800 animate-pulse
```
...
| Focus indicator      | Visible focus ring required       |
...
| secondary   | `bg-neutral-800 border border-neutral-700 hover:bg-neutral-700`         |
...
| Dialog         | `bg-neutral-800` | `border-neutral-700` |
| DialogOverlay  | `bg-neutral-900/80` | —                 |
| Popover        | `bg-neutral-800` | `border-neutral-700` |
| Select         | `bg-neutral-800` | `border-neutral-700` |
| DropdownMenu   | `bg-neutral-800` | `border-neutral-700` |

### Focus States (All Components)

| Item        | Class                               |
|-------------|-------------------------------------|
| Focus item  | `focus:bg-neutral-700`              |
...
| Item              | Reason                  |
|-------------------|-------------------------|
| `shadow-*`        | Không dùng shadow       |
| `gradient-*`      | Không dùng gradient     |
| `backdrop-blur`   | Không dùng glass effect |
| Emoji             | Không dùng trong UI     |
| Bounce animation  | Quá flashy              |
| Neon colors       | Không phù hợp palette   |
| Filled icons      | Chỉ dùng outline icons  |
...
> **Approach:** Mobile-first — viết style cho mobile trước, dùng prefix để scale up.
...
| Layout Type    | Mobile         | Tablet (`md:`)    | Desktop (`lg:`)   |
|----------------|----------------|-------------------|-------------------|
| Card grid      | `grid-cols-1`  | `grid-cols-2`     | `grid-cols-3`     |
| Product grid   | `grid-cols-2`  | `grid-cols-3`     | `grid-cols-4`     |
| Dashboard      | `grid-cols-1`  | `grid-cols-2`     | `grid-cols-4`     |
| Form layout    | `grid-cols-1`  | `grid-cols-2`     | `grid-cols-2`     |
...
|------------------|---------------|--------------------|
| Sidebar + Main   | `flex-col`    | `lg:flex-row`      |
| Hero section     | `flex-col`    | `md:flex-row`      |
| Footer columns   | `flex-col`    | `md:flex-row`      |
| Nav items        | `flex-col`    | `lg:flex-row`      |
...
| Class                    | Behavior                      |
|--------------------------|-------------------------------|
| `hidden md:block`        | Ẩn mobile, hiện từ tablet     |
| `block md:hidden`        | Hiện mobile, ẩn từ tablet     |
| `hidden lg:flex`         | Ẩn mobile/tablet, flex desktop|
| `md:hidden`              | Ẩn từ tablet trở lên          |
...
| Size   | Mobile           | Desktop (`md:`)    |
|--------|------------------|--------------------|
| CTA    | `w-full h-12`    | `md:w-auto h-10`   |
| Icon   | `w-10 h-10`      | `md:w-8 md:h-8`    |

#### Modal / Dialog

| Size   | Mobile           | Tablet+ (`md:`)    |
|--------|------------------|--------------------|
...
```jsx
// Responsive modal
<DialogContent className="w-full md:max-w-lg mx-4 md:mx-auto">
  ...
</DialogContent>
```
...
| Aspect      | Mobile          | Desktop (`lg:`)     |
|-------------|-----------------|---------------------|
| Hero        | `aspect-square` | `lg:aspect-[16/9]`  |
| Card thumb  | `aspect-video`  | `aspect-video`      |
| Avatar      | `w-10 h-10`     | `lg:w-12 lg:h-12`   |

```jsx
<img 
  className="w-full aspect-square lg:aspect-[16/9] object-cover rounded-xl" 
  src="..." 
  alt="..." 
/>
```
...
| Pattern        | Mobile          | Desktop (`md:`)    |
|----------------|-----------------|---------------------|
| Hero heading   | `text-center`   | `md:text-left`      |
| Footer         | `text-center`   | `md:text-left`      |
| Stats          | `text-center`   | `text-center`       |
...
| Full → Auto width    | `w-full md:w-auto`                               |
| Center → Left        | `text-center md:text-left`                       |
| 1 col → 3 cols       | `grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3` |
...
| Rule                          | Description                                      |
|-------------------------------|--------------------------------------------------|
| Mobile-first                  | Viết base style cho mobile, scale up với prefix  |
| Touch targets                 | Min `44x44px` cho tất cả interactive elements    |
| Readable text                 | Min `16px` body text trên mobile (tránh zoom)    |
| No horizontal scroll          | Luôn test để tránh overflow-x                    |
| Test real devices             | Không chỉ dựa vào DevTools                       |
| Limit breakpoints             | Chỉ dùng 2-3 breakpoints cho mỗi component       |

---

> **Tip:** Ưu tiên dùng `md:` và `lg:` là chính. Tránh dùng quá nhiều breakpoints gây phức tạp.

---

> **Version:** 2.1  
> **Stack:** Tailwind CSS + solid/ui + Lucide React
