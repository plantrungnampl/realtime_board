...
-- ============================================================================
CREATE TABLE core.organization_invite (
    id                  UUID PRIMARY KEY DEFAULT uuid_generate_v7(),
    ...
    invite_token        VARCHAR(64), -- nullable (deprecated, plaintext removed)
    ...
    -- Constraints
    CONSTRAINT org_invite_unique UNIQUE (organization_id, email)
);

-- Indexes
CREATE INDEX idx_org_invite_org ON core.organization_invite(organization_id);
CREATE INDEX idx_org_invite_email ON core.organization_invite(email);
CREATE INDEX idx_org_invite_token_hash ON core.organization_invite(invite_token_hash);
CREATE INDEX idx_org_invite_token ON core.organization_invite(invite_token);
```
...
CREATE TABLE board.board_member (
    ...
    custom_permissions  JSONB,  -- Override default role permissions (e.g., {"canView": true, "canEdit": false})
...
-- ============================================================================
-- ELEMENT WRITE SERIALIZATION (PER BOARD)
-- ============================================================================
CREATE OR REPLACE FUNCTION board.lock_element_writes()
RETURNS TRIGGER AS $$
DECLARE
    target_board_id UUID;
BEGIN
    target_board_id := COALESCE(NEW.board_id, OLD.board_id);
    PERFORM pg_advisory_xact_lock(hashtextextended(target_board_id::text, 0));
    RETURN COALESCE(NEW, OLD);
END;
$$ LANGUAGE plpgsql;

CREATE TRIGGER element_board_lock
    BEFORE INSERT OR UPDATE OR DELETE ON board.element
    FOR EACH ROW EXECUTE FUNCTION board.lock_element_writes();
...
RETURNS TRIGGER AS $$
BEGIN
    IF current_setting('app.crdt_projection', true) = 'on' THEN
        RETURN NEW;
    END IF;
...
BEGIN
    IF TG_OP = 'INSERT' THEN
        ...
        _actor_id := NULLIF(_new_data->>'created_by', '')::uuid;
        IF _actor_id IS NULL THEN
            _actor_id := NULLIF(current_setting('app.current_user_id', true), '')::uuid;
        END IF;
...
CREATE TRIGGER audit_board_member_changes
    AFTER INSERT OR UPDATE OR DELETE ON board.board_member
    FOR EACH ROW EXECUTE FUNCTION audit.log_changes();
