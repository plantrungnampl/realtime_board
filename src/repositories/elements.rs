...
pub async fn find_element_by_id(
    ...
    let element = sqlx::query_as::<_, BoardElement>(
        r#"
            SELECT *
            FROM board.element
            WHERE id = $1
...
pub async fn list_element_versions(
    ...
    let elements = sqlx::query_as::<_, ElementVersionRow>(
        r#"
            SELECT id, version
            FROM board.element
            WHERE board_id = $1
...
pub async fn list_elements_by_board(
    ...
    let elements = sqlx::query_as::<_, BoardElement>(
        r#"
            SELECT *
            FROM board.element
            WHERE board_id = $1
...
pub async fn list_projection_defaults(
    ...
    let rows = sqlx::query_as::<_, ElementProjectionDefaults>(
        r#"
